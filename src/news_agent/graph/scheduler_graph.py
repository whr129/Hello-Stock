...
def build_scheduler_graph(session_factory: async_sessionmaker, settings: Settings):
    ...
    graph.add_node("load_due_sources", nodes.load_due_sources)
    graph.add_node("fetch_parallel", nodes.fetch_parallel)
    graph.add_node("normalize_dedupe", nodes.normalize_dedupe)
    graph.add_node("embed_store", nodes.embed_store)
    graph.add_node("precompute_summaries", nodes.precompute_summaries)
    graph.add_node("quality_check", nodes.quality_check)
    graph.add_node("retry_or_recover", nodes.retry_or_recover)
